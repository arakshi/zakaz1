...
Также в `build_model_3d.java` и `build_model_2d_axi.java` standalone `main()` больше не зависят
от `run_sweeps.defaultParams()`, чтобы исключить ошибку компиляции
`run_sweeps cannot be resolved` при агрессивных режимах `comsolcompile`.


Если в `compile*.log` видите ошибку вида `<> operator is not allowed for source level below 1.7`,
используйте текущую версию репозитория: в Java-файлах убраны diamond-операторы и
исправлены `main()`-сигнатуры для checked-exceptions (`throws Exception`).


Для старого Java source level в COMSOL также убраны try-with-resources в `postprocess.java` и
приведение результатов `getReal()` сделано через `double[][]` -> scalar, чтобы избежать
ошибок типов при компиляции.
