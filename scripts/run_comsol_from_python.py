...
def run_compile_and_batch(comsol_path: str, project_dir: Path, dry_run: bool, prefer_direct_tools: bool) -> None:
    ...
    if compile_tool and batch_tool:
        ...
        for java_file in JAVA_FILES:
            run_cmd([compile_tool, "-inputfile", java_file, "-outputfile", build_out], cwd=project_dir, dry_run=dry_run)
        run_cmd([batch_tool, "-classpath", "build", "-inputfile", "src/run_sweeps.java", "-outputfile", "results/log.txt"], cwd=project_dir, dry_run=dry_run)
        return
