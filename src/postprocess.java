...
public class postprocess {
  ...
  private static double firstValue(double[][] a) {
    if (a == null || a.length == 0 || a[0].length == 0) return Double.NaN;
    return a[0][0];
  }

  public static Metrics evaluate(Model model) {
    Metrics m = new Metrics();
    m.tEval = model.param().evaluate("Npulses/f");
    ...
    m.tMax = firstValue(model.result().numerical("maxV").getReal());
    m.tMin = firstValue(model.result().numerical("minV").getReal());
    m.tMean = firstValue(model.result().numerical("avV").getReal());
    ...
    double var = firstValue(model.result().numerical("avVar").getReal());
